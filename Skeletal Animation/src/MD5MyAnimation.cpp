...
bool MD5MyAnimation::LoadAnimation(const std::string& filename)
{
    if (!fs::exists(filename))
    {
        std::cerr << "MD5MyAnimation::LoadAnimation: Failed to find file: " << filename << std::endl;
        return false;
    }
...
void MD5MyAnimation::BuildFrameSkeleton(FrameSkeletonList& skeletons, const JointInfoList& jointInfos, const BaseFrameList& baseFrames, const FrameData& frameData)
{
...
void MD5MyAnimation::Update(float fDeltaTime)
{
...
void MD5MyAnimation::InterpolateSkeletons(FrameSkeleton& finalSkeleton, const FrameSkeleton& skeleton0, const FrameSkeleton& skeleton1, float fInterpolate)
{
    for (int i = 0; i < m_iNumJoints; ++i)
    {
